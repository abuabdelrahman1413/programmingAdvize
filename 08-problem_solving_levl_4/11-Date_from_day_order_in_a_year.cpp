...
sDate GetFormatedDateFromOrderdDayInAYear(short Dayorderd, short Year)
{
    sDate Date;
    Date.Year = Year;
    Date.Month = 1;
    short RemainingDays = Dayorderd;

    while (true)
    {
        if (RemainingDays > NumberOfDaysInMonth(Year, Date.Month))
        {
...
int main()
{
    ...
    sDate Date;
    Date = GetFormatedDateFromOrderdDayInAYear(day_order, year);
    cout << "\n\nDate of [" << day_order << "] is: ";
